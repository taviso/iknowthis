...
#ifndef SYS_old_execve
# define SYS_old_execve 11
#endif

// Execute program.
// int execve(const char *filename, char *const argv[], char *const envp[]);
SYSFUZZ(execve, SYS_old_execve, SYS_NONE, CLONE_DEFAULT, 1000)
{
    glong       retcode;
    gchar      *pathname;
    ...
    // Execute system call.
    if (g_random_boolean()) {
        retcode = spawn_syscall_lwp(this, NULL, SYS_old_execve, typelib_get_pathname(&pathname), argv, envp);
    } else {
        retcode = spawn_syscall_lwp(this, NULL, SYS_old_execve, typelib_get_pathname(&pathname), *argv, *envp);
    }
